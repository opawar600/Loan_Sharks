---
title: "Logistic Regression for Loansharks (2016 Pennsylvania)"
...
model_summary_and_plot(model_2016_income_loan_amount, train)
mcfadden_r_square(model_2016_income_loan_amount, null_model)
...
```{r}
model_2016_ratio_of_loan_to_income_with_log_of_income_and_amount <- build_glm_model("loan_granted~applicant_race_and_ethnicity+owner_occupancy+preapproval+log_loan_amount_000s+applicant_sex+county_code+tract_to_msamd_income+co_applicant_present+agency_code+minority_population+loan_purpose+rate_spread+property_type+loan_to_income_ratio+respondent_id", train)

model_summary_and_plot(model_2016_ratio_of_loan_to_income_with_log_of_income_and_amount, train)

mcfadden_r_square(model_2016_ratio_of_loan_to_income_with_log_of_income_and_amount, null_model)
...
# Cross validation
```{r}
model_2016_cross_val_loan_income_ratio <- cross_validation("loan_granted~applicant_race_and_ethnicity+owner_occupancy+preapproval+log_loan_amount_000s+applicant_sex+county_code+tract_to_msamd_income+co_applicant_present+agency_code+minority_population+loan_purpose+rate_spread+property_type+loan_to_income_ratio+respondent_id", train)

model_summary_and_plot(model_2016_cross_val_loan_income_ratio$finalModel, train)
...
library(ROCR)
library(pROC)
model1_fitted.probs <- predict(model_2016_income_loan_amount, test, type = "response")
model2_fitted.probs <- predict(model_2016_log_income_loan_amount, test, type = "response")
model3_fitted.probs <- predict(model_2016_ratio_of_loan_to_income_with_log_of_income_and_amount, test, type = "response")
...
model4_fitted.probs <- predict(model_2016_cross_val_loan_income_ratio, test, type = "prob")
writeLines("")
...
```{r}
period_list <- c("2016-01", "2016-02", "2016-03", "2016-04",
...
```{r}
model_2016_with_loan_to_value_ratio <-
...
model_summary_and_plot(model_2016_with_loan_to_value_ratio, train = new_train)
mcfadden_r_square(model_2016_with_loan_to_value_ratio, null_model)
...
model6_fitted.probs <- predict(model_2016_with_loan_to_value_ratio, new_test, type = "response")
...
save(model_2016_income_loan_amount, file = paste(model_path, "/model_income_loan_amount.rdata", sep=""))
save(model_2016_log_income_loan_amount, file = paste(model_path, "/model_log_income_loan_amount.rdata", sep=""))
save(model_2016_ratio_of_loan_to_income_with_log_of_income_and_amount, file = paste(model_path, "/model_loan_income_ratio_with_log_of_income_and_amount.rdata", sep=""))
save(model_2016_cross_val_loan_income_ratio, file = paste(model_path, "/cross_val_model.rdata", sep=""))
save(merged_2016_model, file = paste(model_path, "/merged_model.rdata", sep=""))

save(model_2016_with_loan_to_value_ratio, file = paste(model_path, "/merged_model_with_loan_to_value.rdata", sep=""))
```
