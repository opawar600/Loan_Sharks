---
title: "PA 2014 HMDA Feature Selection"
...
# Load
library(wesanderson)
p<-ggplot(data=var_imp_sorted_top_25, aes(x=no, y = reorder(rownames(var_imp_sorted_top_25),no), fill = no)) +
  geom_bar(stat="identity")
p +scale_fill_gradient(low="steelblue", high="tomato")+labs(title = "Value of that predictor in relation to the output variable", x = "Importance", y = "Variable" , fill = "Importance")
...
p<-ggplot(data=importance_df_top, aes(x=overall, y = reorder(names, overall), fill = overall)) +
  geom_bar(stat="identity") 
p+scale_fill_gradient(low="steelblue", high="tomato")+labs(title = "Variable Importances using full glm model", x = "Importance", y = "Variable" , fill = "Importance")
