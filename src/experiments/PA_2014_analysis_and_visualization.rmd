...
## Load data file.
```{r}
hmda_data_pa <- fread(paste(data_dir, "/2014/hmda_2014_pa_all-records_labels.csv", sep = ""))
```
...
unique(train$co_applicant_present)
```
...
# Model performance.
```{r}
prediction_stats(fitted.probs, test)
```
...
# Model performance.
```{r}
prediction_stats(fitted_2.probs, test)
```
...
# Model performance.
```{r}
prediction_stats(fitted_2_log_income.probs, test)
```
...
confusionMatrix(data=as.factor(pred.results), test$loan_granted, positive = "TRUE")
```
...
confusionMatrix(data=as.factor(pred2.results), test$loan_granted, positive = "TRUE")
```
...
confusionMatrix(data=as.factor(pred2_log_k_fold_loan_income_ratio.results), test$loan_granted, positive = "TRUE")
```
...
confusionMatrix(data=as.factor(pred_merged.results), merged_test$loan_granted, positive = "TRUE")
```
